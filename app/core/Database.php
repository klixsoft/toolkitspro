...
class Database {
     ...
     * @param string $socket
     */
    public function __construct($host = DB_HOST, $username = DB_USER, $password = DB_PASSWORD, $db = DB_NAME, $port = null, $charset = DB_CHARSET, $socket = null)
    {
        //check script is already installed or not
        if( ! $this->has_system_installed() ){
            $this->define_site_path();
            return false;
        }
        ...
        if (defined( "TABLE_PREFIX" ) && !empty( TABLE_PREFIX )) {
     ...
     * Define Site Path
     */
    private function define_site_path(){
        $scriptFile = trim($_SERVER['SCRIPT_NAME'], "/");
        $scriptFile = str_replace("/index.php", "", $scriptFile);
        $scriptFile = trim($scriptFile, "/");
        define("SITEPATH", $scriptFile);
        
        //url
        $url = str_replace($_SERVER['SCRIPT_URL'], "", $_SERVER['SCRIPT_URI']);
        define("SITEURL", $url . "/" . (empty(SITEPATH) ? "" : SITEPATH . "/"));
    }

    /**
     * Check system is already installed or not
     */
     ...
     * @throws Exception
     */
    public function rawQuery($query, $bindParams = null, $addPefix=true)
    {
        if( $addPefix )
     ...
     * @throws Exception
     */
    public function update_row($tableName, $tableData, $numRows = null)
    {
        if ($this->isSubQuery) {
            return;
        }
     ...
     * @throws Exception
     */
    public function delete_row($tableName, $numRows = null)
    {
        if ($this->isSubQuery) {
            return;
        }
    ...
    /**
     * This function get last inserted id of the row
     */
    ...
    protected function _dynamicBindResults(\mysqli_stmt $stmt)
    {
        ...
        while ($field = $meta->fetch_field()) {
            ...
            if ($this->_nestJoin && $field->table != $this->_tableName) {
                $field->table = substr($field->table, mb_strlen($this->prefix));
    ...
    public function select($table, $what = [], $conditions = [], $order = []) {
        if( ! empty( $conditions ) ){
        ...
        return $this->get($table, null, $what);
    }

    public function update($table, $updatevaluesArray, $conditions = []) {
        if( ! empty( $conditions ) ){
            foreach($conditions as $key => $value){
                $this->where($key, $value);
            }
        }
        return $this->update_row($table, $updatevaluesArray);
    }
    ...
    public function delete($table, $conditions){
        if( ! empty( $conditions ) ){
            ...
            return $this->delete_row( $table );
        }
        return false;
    }

    public function deleteByCondition($table, $conditions){
        if( ! empty( $conditions ) ){
            $this->where($conditions[0], $conditions[1]);
            return $this->delete_row( $table );
        }
        return false;
    }
