...
class UploadHandler
{
    ...
    protected function get_unique_filename($file_path, $name, $size, $type, $error,
        ...
        $content_range_value = !empty($content_range ) && is_array($content_range) && count($content_range) > 1 ? (int)$content_range[1] : 0;
        $uploaded_bytes = $this->fix_integer_overflow($content_range_value);
    ...
    public function onUploadEnd ($res){
        ...
        if ($is_img)
        {
            if(isset($this->options['config']['image_watermark']) && $this->options['config']['image_watermark']){
                $magicianObj = new Imagick();
                $magicianObj->readImage($targetFile);
                
                $watermark = new Imagick();
                $watermark->readImage($this->options['config']['image_watermark']);

                $x = 0;
                $y = 0;
                $magicianObj->compositeImage($watermark, Imagick::COMPOSITE_OVER, $x, $y);
                $magicianObj->writeImage($targetFile);
            }
