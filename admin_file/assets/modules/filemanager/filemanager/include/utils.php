...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop",$config = array())
{
	...
	if(file_exists($imgfile) || strpos($imgfile,'http')===0){
		...
		{
			try{
				$magicianObj = new Imagick($imgfile);
				$magicianObj->resizeImage($newwidth, $newheight, Imagick::FILTER_LANCZOS, 1, true);
				$magicianObj->setImageCompressionQuality(80);
				$magicianObj->writeImage($imgthumb);
			}catch (Exception $e){
				print_r($e);
				die();
				return $e->getMessage();
	...
	if (file_exists($img))
	{
		...
		if(ini_get('memory_limit') > 0 ){
			...
			if( pathinfo($img, PATHINFO_EXTENSION) == 'svg' ){
				$svgXML = simplexml_load_file($img);
				list($originX, $originY, $image_width, $image_height) = explode(' ', $svgXML['viewBox']);
			}else{
