$(document).ready(function(){
    ...
    $(document).on("submit", ".youtube-title-generator_submit_form", function(e){
        e.preventDefault();

        const form = $(this);
        const report = form.find(".youtube-title-generator_report");
    ...
    $(document).on("click", ".yt-tags-formatter_copy", function () {
        ...
        if (val && val.length > 0) {
            ...
            $(document.body).trigger("has_message", {
                type: "danger",
                messages: [
                    "Favourite Titles is empty. Please add some Titles to Favourite list!!!"
                ]
            });
        }
    });
    ...
    $(document).on("click", ".yt-tags-formatter_download", function () {
        const val = $(document).find("#tags_result").val();
        if (val && val.length > 0) {
            download_text(val, "titles.txt", "data:text/plain;charset=utf-8");
            $(this).addClass("active");
            setTimeout(() => {
                $(this).removeClass("active");
            }, 1000);
