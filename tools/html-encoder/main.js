$(document).ready(function() {
    ...
    $(document).on("click", ".encodeHTMLCode", function(e) {
        e.preventDefault();
        const originalValue = editor.getValue();
        const textArea = document.createElement("textarea");
        textArea.innerText = originalValue;
        const encodedHTML = textArea.innerHTML.split("<br>").join("\n");
        const outputEle = $(document).find("#html-encoder_result");
        outputEle.val(encodedHTML);
        $("html, body").animate({
            scrollTop: outputEle.closest(".form-group").offset().top - 100
        })
    });
    $(document).on("click", ".html-encoder_copy", function() {
        const val = $(document).find("#html-encoder_result").val();
        if (val && val.length > 0) {
            copy_text(val);
    ...
    $(document).on("click", ".html-encoder_download", function() {
        const val = $(document).find("#html-encoder_result").val();
        if (val && val.length > 0) {
